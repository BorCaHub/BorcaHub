--[[
    ...
    ║   • Scoped Cleanup (Maid pattern per-window/per-tab)                 ║
    ║   • Stealth Layer (Anti-Cheat bypass via gethui/CoreGui)             ║
    ║   • Internal Error Handling (Safe pcall + error categorization)      ║
    ║   • UI Element Tracking & Cleanup                                    ║
    ║   • Action Logger (rate-limited, level-filtered, max-buffer)         ║
    ║   • Auto Garbage Collection (periodic heartbeat-based)               ║
    ║   • Health Monitor (statistik GC historis)                           ║
    ║   • Guard.Once() / Guard.Debounce() utilities                        ║
    ║   • Guard.Destroy() — full module teardown                           ║
    ╚══════════════════════════════════════════════════════════════════════╝

    CHANGELOG v4.1.0:
      + Maid (Scope) — per-window/tab scoped cleanup object
      ...
      + Guard.Once()     — single-fire connection wrapper
      + Guard.Debounce() — debounce wrapper untuk callbacks
...
--- @param message string  — Pesan log
--- @param tag     string? — Opsional kategori tag (e.g. "GC", "MAID")
...
--- @param level string? — Filter opsional berdasarkan level
--- @return table — Array { time, level, msg }
...
--- @param func function — Fungsi yang dieksekusi dengan aman
--- @param ...  any      — Argumen untuk fungsi
--- @return boolean, any — Status sukses dan hasil (atau pesan error)
...
--- @param func function — Callback pengguna
--- @return function — Fungsi yang sudah dibungkus aman
...
--- @param func     function — Fungsi yang ingin di-debounce
--- @param cooldown number?  — Detik cooldown (default: 0.5)
--- @return function — Fungsi terbungkus debounce
...
--- @param silent boolean? — Jika true, tidak cetak log (untuk auto-GC)
...
--- @param interval number? — Interval detik (default: 30)
...
--- @return table — Snapshot statistik Guard saat ini
function Guard.GetHealth()
    return {
...
--- @function Maid:Add
--- @param item any — Instance | RBXScriptConnection | function
--- @return any — Item yang sama (untuk chaining)
...
--- @param item any — Hapus item dari tracking tanpa destroy.
...
---   Connection → Disconnect, Instance → Destroy, function → call.
function Maid:Destroy()
    Guard.Log("INFO",
...
--- @param name string? — Nama opsional untuk debugging
--- @return Maid
...
--- @param gui ScreenGui — UI yang ingin dilindungi
--- @return boolean — True jika berhasil
--- @description Pindahkan UI ke container aman agar tidak terdeteksi
---   anti-cheat yang men-scan PlayerGui.
---
---   Urutan prioritas:
---     1. syn.protect_gui()  — Synapse X native protection
---     2. protect_gui()      — Global executor generik
---     3. gethui()           — Container executor generik
---     4. CoreGui            — Fallback standar Roblox
---     5. Gagal              — Return false, caller harus handle
