--[[
    ...
    ║   + 6 New Accents : Gold · Silver · Emerald · Sky · Coral · Violet   ║
    ║   + Library:Watermark()    — FPS/version overlay                     ║
    ║   + Library:Banner()       — top-screen announcement banner          ║
    ║   + Library:InputPrompt()  — dialog with text input field            ║
    ║   + Library:SetAccent()    — runtime accent color change             ║
    ║   + Library:Toast()        — quick notification shorthand            ║
    ║   + Sec:AddSeparator()     — labeled divider inside sections         ║
    ║   + Sec:AddText()          — informational text block                ║
    ║   + Sec:AddProgressBar()   — updatable progress bar element          ║
    ║   + Tab:Select()           — programmatic tab switching              ║
    ║   + Win:SetTitle()         — runtime window title change             ║
    ║   * Smoother open animation (spring scale + fade)                    ║
    ║   * Logo badge pulse glow effect                                     ║
    ╚══════════════════════════════════════════════════════════════════════╝
--]]
...
pcall(function()
    if isfile and readfile then
        ThemeEngine = loadstring(readfile("BorcaHub/UIMAIN/COMPONENTS/CORE/THEME.txt"))()
        Guard       = loadstring(readfile("BorcaHub/UIMAIN/COMPONENTS/CORE/GUARD.txt"))()
    end
end)
...
function Library:CreateWindow(opt)
    ...
    local UserTierLbl = New("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Size                 = UDim2.new(1, 0, 0, 12),
        Text                 = "✦ Free",  -- default Free, diubah oleh loader
        TextColor3           = T.TextSub,
        TextSize             = 10,
        Font                 = Enum.Font.GothamBold,
        TextXAlignment       = Enum.TextXAlignment.Left,
    ...
    -- ── SetUserInfo: dipanggil oleh loader untuk set nama & tier ──
    ...
    -- ── OPEN ANIMATION (v3.0.0 — spring scale + fade) ───────────────
...
function Library.SectionElements:AddLineChart(chartOpt)
    chartOpt = chartOpt or {}
    local T = ThemeEngine.Themes[Library.ActiveTheme] or ThemeEngine.Themes.Dark
    ...
    local function redraw()
        ...
        for i, v in ipairs(data) do
            ...
            if prevPoint then
                local dist = math.sqrt(math.pow(px - prevPoint.X, 2) + math.pow(py - prevPoint.Y, 2))
...
function Library.SectionElements:AddNodeEditor(nodeOpt)
    nodeOpt = nodeOpt or {}
    local T = ThemeEngine.Themes[Library.ActiveTheme] or ThemeEngine.Themes.Dark
...
function Library.SectionElements:AddTreeView(treeOpt)
    treeOpt = treeOpt or {}
    local T = ThemeEngine.Themes[Library.ActiveTheme] or ThemeEngine.Themes.Dark
...
return Library — TIDAK ada eksekusi standalone !!
...
function Library:CreateCodeEditor(opt)
    ...
    -- ── Body (Line Numbers + Code Area + Minimap) ──
    local Body = New("Frame", {
        Parent                 = EditorFrame,
        BackgroundTransparency = 1,
        Position               = UDim2.new(0, 0, 0, 34),
