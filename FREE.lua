--[[
    ...
    2. Enforce strict cleanup of any Key Prompt UI
    3. Load the Game Checker (SCRIPTS/CHECKGAMEID.lua)
    4. Detect current game via PlaceId
    5. Load the matching FREE script
    6. If game not specifically supported, auto-load Universal Free
--]]
...
local FreeLoader = {}
FreeLoader.Version = "3.5.0-FREE"
FreeLoader.BuildDate = "2026-05-25"
FreeLoader.SessionID = HttpService:GenerateGUID(false)
FreeLoader.LoadStart = tick()
...
ResourceCore.OptimizeMemory()

-- ================================================================
--  [ AGGRESSIVE UI CLEANUP SYSTEM ]
-- ================================================================
local function CleanupOldUI()
    ...
    pcall(function() totalCleaned = totalCleaned + clean(CoreGui) end)
    
    if LocalPlayer then
        local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
        if playerGui then 
            pcall(function() totalCleaned = totalCleaned + clean(playerGui) end) 
        end
    end
    
    if totalCleaned > 0 then
        print("[BorcaHub FREE] Swept " .. tostring(totalCleaned) .. " legacy UI elements.")
    end
end

-- Ensure no key prompts remain from previous sessions
CleanupOldUI()
...
-- ================================================================
--  [ LOAD GAME CHECKER MODULE ]
-- ================================================================
local CheckGameID
do
    local function loadModule()
        local path = "BorcaHub/SCRIPTMAIN/SCRIPTS/CHECKGAMEID.lua"
        ...
        if not ok or not content then return nil, "File read error" end
    ...
    local result, errStr = loadModule()
    if result and type(result) == "table" and result.Validate then
        CheckGameID = result
    else
        Notify("BorcaHub Error", "Failed to load Game Checker module.", 10)
        warn("[BorcaHub FREE] Game Checker load error:", errStr or tostring(result))
        return
    end
end

-- ================================================================
--  [ DETECT GAME & EXECUTE SCRIPT ]
-- ================================================================
if CheckGameID.DebugPrint then
    pcall(CheckGameID.DebugPrint, "Free")
end
...
if not validation.FileExists then
    ...
    if uniExists then
        ...
        if ok then
            ...
            warn("[BorcaHub FREE] Error in Universal script:", msg)
        end
    else
        Notify("BorcaHub Error", "CRITICAL: Universal script missing.", 10)
        warn("[BorcaHub FREE] Installation corrupted. Missing Universal fallback.")
    end
    return
end
