...
local function IsEnemy(player)
    ...
    -- In Rivals, check team; treat all non-local players as potential targets
...
local function NoRecoilLoop()
    ...
    Connections["NoRecoil"] = RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            local char = GetCharacter()
            if char then
                -- Attempt to neutralize recoil by finding gun-related scripts/values
                for _, tool in ipairs(char:GetChildren()) do
                    if tool:IsA("Tool") then
                        -- Search for recoil values in the tool hierarchy
                ...
                -- Also scan PlayerGui for recoil-related modules
                local pg = LocalPlayer:FindFirstChild("PlayerGui")
                if pg then
                    for _, desc in ipairs(pg:GetDescendants()) do
                        if desc:IsA("NumberValue") then
                            local nameLower = desc.Name:lower()
                            if nameLower:find("recoil") or nameLower:find("spread") then
                                desc.Value = 0
                            end
                        end
                    end
...
local function CreateESPBox(player)
    ...
    pcall(function()
        ...
        for _, line in pairs(espData) do
            if typeof(line) == "table" or (line and line.Color) then
                -- Drawing object
            end
        end
            ...
            if player ~= LocalPlayer then
                if States.ESPEnabled then
                    ...
                    pcall(function()
                        ...
                        -- Determine color based on team/enemy
                        local boxColor = Color3.fromRGB(255, 50, 50) -- Red for enemies
                        if player.Team and LocalPlayer.Team and player.Team == LocalPlayer.Team then
                            boxColor = Color3.fromRGB(50, 255, 50) -- Green for teammates
                        end
...
local function BoostFPS()
    pcall(function()
        ...
        -- Remove unnecessary decals and textures from workspace
        ...
        SafeNotify("BorcaHub", "FPS Boost & Anti-Lag applied!")
    end)
end
...
InfoSection:AddLabel("Game: Rivals (FPS)")
...
ServerSection:AddButton({
    Name = "Rejoin Server",
    Callback = function()
        SafeNotify("BorcaHub", "Rejoining server...")
        task.wait(1)
        RejoinServer()
    end
})
...
ServerSection:AddButton({
    Name = "Hop Server",
    Callback = function()
        SafeNotify("BorcaHub", "Hopping to a new server...")
        task.wait(1)
        HopServer()
    end
})
...
ServerSection:AddButton({
    Name = "Change Game",
    Callback = function()
        SafeNotify("BorcaHub", "Use the Place Game ID input above, then click this button")
    end
})

GameSection:AddInput({
    Name = "Enter Place ID to Teleport",
    Callback = function(value)
        ChangeGame(value)
    end
})
...
MiscSection:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            SetupAntiAFK()
            SafeNotify("BorcaHub", "Anti-AFK Enabled")
        else
            if Connections["AntiAFK"] then
                Connections["AntiAFK"]:Disconnect()
...
MiscSection:AddLabel("BorcaHub | Rivals FREE v0.0.1")
...
-- Handle player removing (cleanup ESP)
...
-- Handle character respawn (re-apply settings)
...
-- ═══════════════════════════════════════════════════════════
SafeNotify("BorcaHub | Rivals FREE", "Script loaded successfully! v0.0.1")

print("[BorcaHub] Rivals FREE v0.0.1 loaded successfully")
print("[BorcaHub] Developer: BORCA")
print("[BorcaHub] Edition: FREE")
