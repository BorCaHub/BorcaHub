--[[
    ╔══════════════════════════════════════════════════════════╗
    ║             BORCAHUB | UNIVERSAL PREMIUM                ║
    ║           Version: 0.0.1 | Developer: BORCA             ║
    ║              Last Update: 2026-05-25                     ║
    ║                                                          ║
    ║    PREMIUM Edition - Full access to all features         ║
    ║    Game: Universal Fallback Script                       ║
    ╚══════════════════════════════════════════════════════════╝
]]

local PREMIUM_KEY = "BORCA-PREMIUM-UNI-2026"
...
local Window = Library:CreateWindow({
    Title = "BorcaHub | Universal Premium",
...
-- CLICK TELEPORT
-- ═══════════════════════════════════════════════════════════
Connections["ClickTP"] = UserInputService.InputBegan:Connect(function(input, processed)
    if processed then return end
    if States.ClickTPEnabled and input.UserInputType == Enum.UserInputType.MouseButton1 and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
        pcall(function()
        ...
        for _, part in ipairs(Workspace:GetDescendants()) do
            if part:IsA("BasePart") and not Players:GetPlayerFromCharacter(part.Parent) and not Players:GetPlayerFromCharacter(part.Parent and part.Parent.Parent) then
                if part.Transparency < 0.5 and part.Name ~= "Baseplate" and part.Name ~= "Terrain" then
                    if not WallhackOriginals[part] then WallhackOriginals[part] = part.Transparency end
...
local function ApplySpeed()
    pcall(function()
        local hum = GetHumanoid()
        if hum then
            if States.SpeedEnabled then hum.WalkSpeed = States.SpeedValue else hum.WalkSpeed = OriginalWalkSpeed end
        end
    end)
end
...
Connections["JumpLoop"] = RunService.Heartbeat:Connect(function()
    if States.JumpBoostEnabled then
        pcall(function()
            local hum = GetHumanoid()
            if hum then
                ...
                pcall(function()
                    ...
                    if States.ESPChamsEnabled then
                        ...
                        if not cham or cham.Parent ~= char then
                            ...
                            cham.FillColor = Color3.fromRGB(255, 255, 0)
                    ...
                    if topOnScreen or bottomOnScreen then
                        ...
                        esp.TopLine.From = topLeft; esp.TopLine.To = topRight; esp.TopLine.Visible = true
                        esp.BottomLine.From = bottomLeft; esp.BottomLine.To = bottomRight; esp.BottomLine.Visible = true
                        esp.LeftLine.From = topLeft; esp.LeftLine.To = bottomLeft; esp.LeftLine.Visible = true
                        esp.RightLine.From = topRight; esp.RightLine.To = bottomRight; esp.RightLine.Visible = true
...
InfoSection:AddLabel("Game: Universal Fallback")
...
local AimSec = MainTab:CreateSection("Aimbot & Teleport")
AimSec:AddToggle({ Name = "Enable Aimbot (RMB)", Callback = function(val) States.AimbotEnabled = val end })
AimSec:AddSlider({ Name = "FOV Size", Min = 20, Max = 500, Default = 120, Callback = function(val) States.AimbotFOV = val end })
AimSec:AddToggle({ Name = "Ctrl + Click Teleport", Callback = function(val) States.ClickTPEnabled = val end })
...
MovementSec:AddToggle({ Name = "Fly Mode", Callback = function(val) States.FlyEnabled = val; if val then StartFly() else StopFly() end end })
MovementSec:AddToggle({ Name = "Noclip Walls", Callback = function(val) States.NoclipEnabled = val end })

local PhysicalSec = PlayerTab:CreateSection("Physical Attributes")
PhysicalSec:AddToggle({ Name = "Speedhack", Callback = function(val) States.SpeedEnabled = val; ApplySpeed() end })
PhysicalSec:AddSlider({ Name = "Speed Value", Min = 16, Max = 250, Default = 32, Callback = function(val) States.SpeedValue = val; ApplySpeed() end })
PhysicalSec:AddToggle({ Name = "Jump Boost", Callback = function(val) States.JumpBoostEnabled = val; ApplyJump() end })
PhysicalSec:AddSlider({ Name = "Jump Value", Min = 50, Max = 350, Default = 80, Callback = function(val) States.JumpValue = val; ApplyJump() end })
PhysicalSec:AddToggle({ Name = "Infinite Jump", Callback = function(val) States.InfiniteJumpEnabled = val end })

-- ═══════════════════════════════════════════════════════════
-- TAB: VISUAL
-- ═══════════════════════════════════════════════════════════
local VisualTab = Window:CreateTab("VISUAL", "rbxassetid://6023426915")
local VisualSec = VisualTab:CreateSection("ESP Options")
VisualSec:AddToggle({ Name = "Box ESP", Callback = function(val) States.ESPEnabled = val end })
VisualSec:AddToggle({ Name = "Chams 3D", Callback = function(val) States.ESPChamsEnabled = val end })
