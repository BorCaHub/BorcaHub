...
local PREMIUM_KEY = "BORCA-PREMIUM-ARSENAL-2026"
...
local Window = Library:CreateWindow({
    Title = "BorcaHub | Arsenal Premium",
...
-- ═══════════════════════════════════════════════════════════
-- RAPID FIRE, NO RECOIL, NO SPREAD, KNIFE AURA
-- ═══════════════════════════════════════════════════════════
Connections["GunMods"] = RunService.Heartbeat:Connect(function()
    pcall(function()
        local char = GetCharacter()
        ...
        for _, tool in ipairs(char:GetChildren()) do
            if tool:IsA("Tool") then
                for _, desc in ipairs(tool:GetDescendants()) do
                    ...
                    if States.RapidFireEnabled and desc.Name == "FireRate" then
                        desc.Value = 0.01
                    end
                end
            end
        ...
        if States.KnifeAuraEnabled then
            local root = GetRootPart()
            local knife = char:FindFirstChildOfClass("Tool")
            if knife and knife.Name:lower():find("melee") or knife.Name:lower():find("knife") then
                for _, player in ipairs(Players:GetPlayers()) do
...
-- ═══════════════════════════════════════════════════════════
-- MOVEMENT SYSTEMS (Fly, Speed, Jump, Noclip, BunnyHop)
-- ═══════════════════════════════════════════════════════════
...
Connections["BunnyHop"] = RunService.Heartbeat:Connect(function()
    if States.BunnyHopEnabled then
        pcall(function()
            local hum = GetHumanoid()
            local root = GetRootPart()
            if hum and root and hum.FloorMaterial ~= Enum.Material.Air and UserInputService:IsKeyDown(Enum.KeyCode.W) then
        ...
        if player ~= LocalPlayer then
            if States.ESPEnabled then
                ...
                pcall(function()
                    ...
                    local color = IsEnemy(player) and Color3.fromRGB(255,0,0) or Color3.fromRGB(0,255,0)
...
Connections["ThirdPerson"] = RunService.Heartbeat:Connect(function()
    pcall(function()
        if States.ThirdPersonEnabled then
            LocalPlayer.CameraMode = Enum.CameraMode.Classic
            Camera.CameraSubject = GetCharacter():FindFirstChild("Head")
        end
    end)
end)
...
local AimSec = MainTab:CreateSection("Combat & Gun Modifications")
AimSec:AddToggle({ Name = "Enable Aimbot (RMB)", Callback = function(val) States.AimbotEnabled = val end })
AimSec:AddToggle({ Name = "Rapid Fire Mod", Callback = function(val) States.RapidFireEnabled = val end })
AimSec:AddToggle({ Name = "No Recoil Enabled", Callback = function(val) States.NoRecoilEnabled = val end })
AimSec:AddToggle({ Name = "No Spread Enabled", Callback = function(val) States.NoSpreadEnabled = val end })
AimSec:AddToggle({ Name = "Knife Aura Melee", Callback = function(val) States.KnifeAuraEnabled = val end })

-- ═══════════════════════════════════════════════════════════
-- TAB: PLAYER
-- ═══════════════════════════════════════════════════════════
local PlayerTab = Window:CreateTab("PLAYER", "rbxassetid://6023426915")
local MoveSec = PlayerTab:CreateSection("Physical Options")
MoveSec:AddToggle({ Name = "Fly Mode Enabled", Callback = function(val) States.FlyEnabled = val; if val then StartFly() else StopFly() end end })
MoveSec:AddToggle({ Name = "Noclip Through Walls", Callback = function(val) States.NoclipEnabled = val end })
MoveSec:AddToggle({ Name = "Speedhack Active", Callback = function(val) States.SpeedEnabled = val end })
MoveSec:AddSlider({ Name = "Speed Value", Min = 16, Max = 250, Default = 32, Callback = function(val) States.SpeedValue = val end })
MoveSec:AddToggle({ Name = "Jump Boost Active", Callback = function(val) States.JumpBoostEnabled = val end })
MoveSec:AddSlider({ Name = "Jump Height", Min = 50, Max = 350, Default = 80, Callback = function(val) States.JumpValue = val end })
MoveSec:AddToggle({ Name = "Bunny Hop Mode", Callback = function(val) States.BunnyHopEnabled = val end })
...
VisSec:AddToggle({ Name = "Third Person View", Callback = function(val) States.ThirdPersonEnabled = val end })
